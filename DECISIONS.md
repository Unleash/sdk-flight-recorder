...
The count accumulates over the window, so it is not a `record()`-time field. The event lifecycle is a chain of progressive types, each guaranteeing a field the previous stage lacks: `ImpressionEvent | CustomEvent` (record input, no `timestamp`/count) → `StampedEvent` (`+ timestamp`, stamped in `record()`) → `WireEvent` (`+ occurrenceCount`, resolved at `drain()`). `WireEvent = DrainedEvent<StampedEvent>`. *Why keep three types, not one optional-field union:* one type spanning all stages would mark `timestamp`/`occurrenceCount` optional, letting code read `occurrenceCount` off an event that has not been drained — a lie the compiler should catch. The chain encodes "which fields are guaranteed when."

## `EventBuffer` storage and the `DrainedEvent<T>` shape

`EventBuffer<T extends object>` holds a single `Map<string, { event: T; occurrenceCount: number }>` keyed by `dedupKey(event)`. There is no parallel `events: T[]` array and no separate `seen` set. `add()` does one `Map.get`: a hit increments `entry.occurrenceCount` in place (no re-`set`, no key rehash) and returns `'duplicate'`; a miss either trips `maxSize` (`'overflow'`) or stores a fresh `{ event, occurrenceCount: 1 }` (`'added'`). `drain()` flattens each stored pair into `DrainedEvent<T> = T & { occurrenceCount: number }` via `Array.from(values, …)` then `clear()` — no per-event key recomputation. `size` is `this.events.size`. Map insertion order preserves first-seen record order on the wire.

*Why the count sits beside the event in storage but is flattened onto it at drain:* the Map value keeps `occurrenceCount` as a sibling of `event` (not a field on it) so a duplicate bumps the counter in place — folding it onto the event instead would force a `{ ...event, occurrenceCount: n+1 }` re-spread per duplicate, an allocation on the hot path. That sibling pair is a private *storage* concern. `drain()` runs once per flush (not the hot path), so it is the right place to spread the count onto the event and hand back the flat `DrainedEvent<T>` that serializes directly to a ClickHouse row — no separate recorder-side merge step, no public wrapper type. `T extends object` is required so the spread is well-typed.
...
| Workload (add 10k + drain) | Two structures (array + count map, recompute at drain) | One Map (flatten at drain) | Change |
|---|---|---|---|
| 10k distinct events | ~103 ops/s | ~174 ops/s | **~1.7× faster** |
| 10k events, 95% duplicates | ~220 ops/s | ~221 ops/s | ~flat |

The distinct workload speeds up because it drains 10k unique events — the old `drain()` paid 10k redundant `dedupKey` (`JSON.stringify`) calls there; the new one pays zero. Both versions still allocate one flat object per drained event (the `{ ...event, occurrenceCount }` spread); in the unified design that spread lives inside `drain()` (and so is captured by this `add + drain` benchmark), whereas before it lived in the recorder's post-drain `.map()` (outside the benchmark) — so the win is the eliminated recompute, not the allocation, which happens either way. The 95%-duplicate workload is flat: it's dominated by the `add()` duplicate fast-path (one `get` + in-place `++`), which the change leaves untouched, and it drains only ~500 distinct events so the drain cost is small. The field-list-vs-replacer ~2× key-builder speedup (separate decision) is preserved across both. Absolute throughput is machine-specific; the ratios are the durable claim — re-measure with `npm run bench`.
