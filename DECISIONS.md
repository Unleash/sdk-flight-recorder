...
`EventBuffer<T extends { occurrenceCount: number }>` holds a single `Map<string, T>` keyed by `dedupKey(event)`, where `T` is the wire shape itself (`WireEvent` in the recorder). There is no parallel `events: T[]` array and no separate `seen` set. `add(event)` does one `Map.get`: a hit folds the incoming count into the buffered event in place (`entry.occurrenceCount += event.occurrenceCount`) and returns `'duplicate'`; a miss either trips `maxSize` (`'overflow'`) or stores the event object as-is (`'added'`). `drain()` is `Array.from(values)` + `clear()` — it hands back the stored objects untouched, **no per-event transform and no key recomputation**. `size` is `this.events.size`. Map insertion order preserves first-seen record order on the wire.
