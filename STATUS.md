...
- `FlightRecorder` with explicit DI: `{ url, clientKey, fetch, batch? }`. Collaborators required, no defaults; `batch.flushAt` opt-in.
...
- Types: `ImpressionEvent` (discriminated by `eventType: 'isEnabled' | 'getVariant'`), `CustomEvent` (`eventType: 'custom'`), `FlightRecorderOptions` (with nested `batch?: { flushAt? }` matching reference structure).
...
6. `'flushes automatically when the buffer reaches the configured size'` — size-based auto-flush via `batch.flushAt`. Asserts: nothing sent below threshold; at threshold, exactly one POST whose body contains both events as NDJSON in order. Subsumes the prior dedicated batching-coverage test.

`src/ndjson.test.ts`
7. `'emits one JSON object per line with a trailing newline'`
...
- **Time-based auto-flush** (`batch.flushAfterMs`). Size threshold landed; timer-based drain not yet pinned. Needs a clock seam.
