...
): ImpressionEvent => ({ ...defaultImpression, ...overrides });
...
describe('semanticEventKey', () => {
    ...
    it('distinguishes isEnabled evaluations from getVariant evaluations', () => {
        ...
        expect(semanticEventKey(isEnabled)).not.toBe(
            semanticEventKey(getVariant),
        );
    });
    ...
    it('never collides an impression with a custom event', () => {
        ...
        expect(semanticEventKey(impression)).not.toBe(
            semanticEventKey(custom),
        );
    });
});
